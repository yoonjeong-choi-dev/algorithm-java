...
public class Prob713SubarrayProductLessThanK {
    public int numSubarrayProductLessThanK(int[] nums, int k) {
        //return bruteForceSolution(nums, k);
        return twoPointerSolution(nums, k);
    }

    private int twoPointerSolution(int[] nums, int k) {
        // curProduct >= k 조건을 사용하기 위해서 k>1 이어야 함
        // 1 <= nums[i] <= 1000 => 곱은 항상 1 초과
        if (k < 2) return 0;
        ...
        for (int end = 0; end < nums.length; end++) {
            ...
            // 타겟보다 작아질 때까지 시작 인덱스 이동
            while (curProduct >= k) {
                curProduct /= nums[start++];
            }

            // [start:end], [start+1:end],.. [end:end] 까지 모두 유효한 배열
            ans += end - start + 1;
        }
    ...
    private int bruteForceSolution(int[] nums, int k) {
        int len = nums.length;
    ...
    public static void main(String[] args) {
        Prob713SubarrayProductLessThanK sol = new Prob713SubarrayProductLessThanK();
        ...
        int[] targets = {100, 0, 19, 9931};

        int[] ans = {8, 0, 18, 4370};
        int numProblems = ans.length;
        for (int i = 0; i < numProblems; i++) {
            System.out.printf("============= Problem %d : \n", i);
            int mySol = sol.numSubarrayProductLessThanK(nums[i], targets[i]);
            if (mySol == ans[i]) {
