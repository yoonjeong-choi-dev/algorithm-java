...
public class JedisIndex {
    ...
    // 트랜잭션을 이용하여 getCounts 성능 향상
    public Map<String, Integer> getCounts(String term) {
        // term 검색어가 등장하는 모든 url 검색
        Set<String> urlsSet = getURLs(term);

        // url set 순회 속도를 빠르게 하기 위해 ArrayList 자료구조로 변환
        List<String> urls = new ArrayList<>(urlsSet);

        // 트랜잭션을 이용하여 각 url 페이지에 등장하는 TermCounter 정보 가져오기
        Transaction transaction = jedis.multi();
        for(String url : urls) {
            String termKey = termCounterKey(url);
            transaction.hget(termKey, term);
        }

        List<Object> countInfo = transaction.exec();

        // 트랜잭션에서 가져온 정보 반환
        Map<String, Integer> ret = new HashMap<>();
        int curIdx = 0;
        for(String url : urls) {
            Integer count = new Integer((String) countInfo.get(curIdx++));
            ret.put(url, count);
        }
    ...
    // 크롤링을 위한 유틸 메서드(added ch15)
    public boolean isIndexed(String url) {
        return jedis.exists((termCounterKey(url)));
    }
    ...
    // Legacy
    public Map<String, Integer> getCountsLegacy(String term) {
        // term 검색어가 등장하는 모든 url 검색
        Set<String> urls = getURLs(term);
